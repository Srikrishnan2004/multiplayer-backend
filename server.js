...
chatNameSpace.on("connection", (socket) => {
  ...
  socket.on("joinVoiceRoom", (roomCode) => {
    ...
    const existingPeers = [
      ...(chatNameSpace.adapter.rooms.get(roomCode) || []),
...
updateNameSpace.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (roomCode) => {
    console.log(
      `[Server] Attempting to join room ${roomCode} by socket ${socket.id}`
    );
    ...
    if (
      gameRooms.get(roomCode).wishlist &&
      gameRooms.get(roomCode).wishlist.length > 0
    ) {
  ...
  socket.on("createRoom", () => {
    console.log(
      `[Server] Received createRoom request from socket ${socket.id}`
    );
    ...
    socket.join(newCode);
    console.log(
      `[Server] Emitting generateCode event to socket ${socket.id} with code ${newCode}`
    );
  ...
  socket.on("updateWishlist", (wishlist) => {
    ...
    if (!socket.userData.roomCode) {
      console.log(
        `Socket ${socket.id} is not in a room. Ignoring wishlist update.`
      );
      return;
    }
  ...
  socket.on("updatePlayer", (player) => {
    if (!socket.userData.roomCode) {
      console.log(
        `[Server] Received update from socket ${socket.id} but no room code set`
      );
      return;
    }
  ...
  setInterval(() => {
    for (const gameRoom of gameRooms.values()) {
      ...
      if (playerData.length > 0) {
        console.log(
          `[Server] Sending player data to room ${gameRoom.roomCode}:`,
          playerData
        );
